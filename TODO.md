...
### 3. investment3 首次加载总成本未换算多币种
- **文件**: `main.py:133`
- **问题**: `page_investment3()` 预加载计算 `total_cost_cny` 时直接累加所有持仓的 `h["total_cost"]`，没有调用 `convert_to_cny()`。如果持有 USD/HKD 等非 CNY 持仓，首次加载总值是错的（各币种数值混加）。刷新后 API 会修正。
- **修法**: 参考 `page_investment()`（第249-257行）的实现，逐个 convert_to_cny 再累加。

### 4. investment_backup.html: catchUpDca 循环永不执行
...
### 4. backtest.html: rapidOpts/dcaOpts 内联样式冲突
...
### 5. api_stats_total 花费公式把投资入金算成支出
- **文件**: `main.py:2188`
- **问题**: `spending = prev_total + income - total`。`total` 不含投资账户余额，因此往投资账户转入 2000 元会使银行余额下降 2000，公式判定为"花了 2000"。实际是转账不是花费。
- **修法**: 投资出入金单独统计，从 spending 中排除；或把投资账户纳入 total。

### 6. 备份导入无事务保护，部分导入后失败会脏数据
...
### 2. typeLabels/typeColors 在4个模板中重复定义
...
### 3. init_db 用原始 sqlite3 连接，路径可能和 SQLAlchemy 不一致
- **文件**: `database.py:26`
...
### 10. api_stats_investment 检查 dca_buy 类型但从未产生
- **文件**: `main.py:2239, 999`
...
### 11. investment_backup.html 是 investment.html 的几乎完整副本
- **文件**: `templates/investment_backup.html` (~951行)
- **问题**: 与 `investment.html` 约90%重复，DCA 表单缺 `payment_account` 字段。每改一个 bug 要改两个文件。
- **修法**: 考虑合并或抽公共 JS。

### 12. avg_cost 在盈利卖出后可能变成负数，显示误导
- **文件**: `main.py:1490, 132, 248`（page_investment3 + api_portfolio）
- **问题**: `avg_cost = total_cost / quantity`，大额盈利卖出后 total_cost 可能为负，导致 avg_cost 出现负值。盈亏计算本身正确，但"平均成本"标签不准确（实际是盈亏平衡价）。
- **修法**: 分两个字段显示：原始平均买入价 和 盈亏平衡价。

---

## 🟢 LOW（小修小补）

### 1. convert_to_cny() 失败时静默返回 0
- **文件**: `exchange_rate.py:134, 138`
- **问题**: 无效币种或汇率获取失败时返回 0，不输出任何警告。可能隐藏数据。
- **修法**: 至少 print 警告，或返回 None 由调用者决定如何处理。

### 2. api_balances 和 api_stats_investment 余额兜底逻辑不一致
- **文件**: `main.py:626,649 vs 2256`
- **问题**: `api_balances` 本月无记录时用上月余额兜底，`api_stats_investment` 直接当 0。口径不一致。
- **修法**: 统一兜底策略。

### 3. _investment_cash() 定义但从未被调用
- **文件**: `main.py:570-587`
- **问题**: 死代码。如果将来调用也缺少多币种转换。
- **修法**: 删除或补充币种转换后接入统计。
